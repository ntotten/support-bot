...
const CHANNELS_KEY = 'support_channels1';
...
export default function(brain, sendMessage) {
  ...
  function shouldEnqueuMessage(channel, message) {
    ...
    if (moment(message.timestamp).subtract(process.env.AUTORESPOND_CONVERSATION_TIMEOUT, 'second').valueOf() < channel.last_agent_message) {
  ...
  function shouldSendReply(channel, message) {
    ...
    if (message.timetamp < moment().subtract(process.env.AUTORESPOND_TIMEOUT, 'second').valueOf()) {
    ...
    if (message.timetamp < moment().subtract(process.env.AUTORESPOND_MINIMUM_REPLY_TIMEOUT, 'second').valueOf()) {
    ...
    if (moment().subtract(process.env.AUTORESPOND_AGENT_WAIT_TIMEOUT, 'second').valueOf() < channel.last_agent_message) {
    ...
    if (lastMessagedUserTime && lastMessagedUserTime > moment().subtract(process.env.AUTORESPOND_USER_LIMIT_TIMEOUT, 'second').valueOf()) {
  ...
  function start() {
    setInterval(runScheduledJob, process.env.AUTORESPOND_JOB_INTERVAL * 1000);
  }
